...
- **Browse-side properties come from Supabase** (the `properties` table, `publish_status = 'live'`), queried via the server client in `src/lib/supabase/server.ts`. Use `src/lib/property-live.ts` (`fetchLiveProperties`, `fetchLivePropertyBySlug`, `mapPropertyRow`) for browse-side queries and DB-row → camelCase `Property` mapping. Do NOT import `properties` from `src/lib/data/properties.ts` for user-facing rendering — that file is the seed fixture only.
- `src/lib/listing-utils.ts` derives `category` (`deriveCategory`) and unique kebab-case `slug` (`uniqueSlug` — appends `-2`/`-3` on collision; the DB `slug` UNIQUE constraint is the backstop). `src/lib/format.ts` (`formatPrice`) derives `price_label` (`" / yr"` for For Rent, `"From "` prefix for Off-Plan).
